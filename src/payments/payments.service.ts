...
export class PaymentsService {
  ...
  async createTransaction(buyerId: string, listingId: string, deliveryMethod: DeliveryMethod = 'main_propre') {
    ...
    // Une seule transaction active par annonce (évite la double vente)
    const active = await this.transactionsRepo.count({
      where: { listingId, status: In(['sequestre', 'livree', 'litige']) },
    });
    ...
    const intent = await this.paymentProvider.createPaymentIntent({
      ...
      applicationFeeEuros: round2(q.commission + q.buyerFee),
      sellerConnectedAccountId,
      metadata: { listingId, buyerId, sellerId: listing.userId },
    });
    ...
    await this.notifications.notify(listing.userId, {
      ...
      body: `Un acheteur a payé « ${listing.title} ». Confirmez la disponibilité et organisez la remise.`,
      link: `/compte/transactions/${transaction.id}`,
    });
  ...
  /** Le code de remise n'est visible que par l'acheteur (il le communique au RDV). */
  private viewFor(tx: Transaction, viewerId: string) {
    const { handoverCode, providerPaymentId, ...rest } = tx;
    return { ...rest, handoverCode: viewerId === tx.buyerId ? handoverCode : undefined };
  }
}
