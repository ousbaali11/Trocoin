    ...
    <div>
      ...
      <div className="a-two">
        <section className="a-panel">
          <h3 style={{ marginTop: 0 }}>Paramètres (appliqués seulement si activée)</h3>
        ...
        <section className="a-panel">
          <h3 style={{ marginTop: 0 }}>Fournisseur de paiement</h3>
      ...
      <h3 style={{ margin: "20px 0 10px" }}>Formules</h3>
      <div className="a-panel" style={{ padding: 0, overflowX: "auto" }}>
        <table className="a-table">
          <thead><tr><th>Nom</th><th>Prix / mois</th><th>Annonces (vide = illimité)</th><th>Mises en avant / mois</th><th>Stats</th><th>Badge</th><th>Vitrine</th><th>Active</th><th>Ordre</th><th></th></tr></thead>
          <tbody>
            {plans.map((p) => (
              <tr key={p.id}>
                <td><input className="a-input" value={p.name} onChange={(e) => setPlan(p.id, { name: e.target.value })} /><input className="a-input" style={{ marginTop: 4 }} value={p.description ?? ""} placeholder="Description" onChange={(e) => setPlan(p.id, { description: e.target.value })} /></td>
                <td><input className="a-input" type="number" step="0.01" style={{ width: 90 }} value={p.priceMonthly} onChange={(e) => setPlan(p.id, { priceMonthly: Number(e.target.value) })} /></td>
                <td><input className="a-input" type="number" style={{ width: 90 }} value={p.listingsIncluded ?? ""} onChange={(e) => setPlan(p.id, { listingsIncluded: e.target.value === "" ? null : Number(e.target.value) })} /></td>
                <td><input className="a-input" type="number" style={{ width: 90 }} value={p.boostsIncluded ?? ""} onChange={(e) => setPlan(p.id, { boostsIncluded: e.target.value === "" ? null : Number(e.target.value) })} /></td>
                <td><input type="checkbox" checked={p.advancedStats} onChange={(e) => setPlan(p.id, { advancedStats: e.target.checked })} /></td>
                <td><input type="checkbox" checked={p.verifiedBadge} onChange={(e) => setPlan(p.id, { verifiedBadge: e.target.checked })} /></td>
                <td><input type="checkbox" checked={p.customShop} onChange={(e) => setPlan(p.id, { customShop: e.target.checked })} /></td>
                <td><input type="checkbox" checked={p.active} onChange={(e) => setPlan(p.id, { active: e.target.checked })} /></td>
                <td><input className="a-input" type="number" style={{ width: 60 }} value={p.sortOrder} onChange={(e) => setPlan(p.id, { sortOrder: Number(e.target.value) })} /></td>
                <td><button className="a-btn primary" disabled={busy} onClick={() => savePlan(p)}>Enregistrer</button></td>
              </tr>
            ))}
          </tbody>
        </table>
